document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('/api/data')
        ...
        .catch(err => {
            console.error('Failed to load data, falling back to empty state or default:', err);
        ...
        if (siteSettings) {
            ...
            if (siteSettings.playUrl) {
                document.querySelectorAll('.play-btn, .game-card a, .btn-random').forEach(el => {
                    if (el.tagName === 'A') el.href = siteSettings.playUrl;
                    // Note: The main random button might have a click handler, so we don't change href if it's not an anchor, 
                    // but if there are other links, update them.
                });
            }
        }
    ...
    // --- Init calls moved to initializeApp() after data fetch ---
    ...
    function startSlotMachineInColumns() {
        ...
        // Re-filter to ensure we have latest data if it changed (though usually static after load)
        const jiliGames = games.filter(g => g.provider === "JILI");
        const pgGames = games.filter(g => g.provider === "PG Soft");
        const ppGames = games.filter(g => g.provider === "PP Slot");
    ...
    function setupCarouselInteraction(container, cards) {
        ...
        cards.forEach((card, index) => {
            ...
            if (index === currentOrder[1]) { // active
    ...
    function createCard(game) {
        ...
        card.innerHTML = `
            <div class="game-image-wrapper">
                <a href="https://www.bigwin959.com/" target="_blank">
